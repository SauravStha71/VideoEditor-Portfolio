...
export default function ProjectCard({ project, index }) {
  ...
  const handleMouseEnter = () => {
    setIsHovered(true);
    if (videoRef.current && project.videoSrc) videoRef.current.play().catch(() => {});
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0" style={{ background: 'var(--bg-surface)' }}>
        {project.videoSrc ? (
          <video ref={videoRef} src={project.videoSrc} muted loop playsInline loading="lazy"
            className="w-full h-full object-cover" poster={project.thumb} />
        ) : (
      ...
      {/* Dark hover overlay — UNCHANGED */}
      <div className={`absolute inset-0 bg-black/50 transition-opacity duration-500 ${isHovered ? 'opacity-100' : 'opacity-0'}`} />
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
            {project.tags.map((tag) => (
              <span key={tag}
                className={`font-mono text-xs px-2 py-1 border border-warm-white/20 text-warm-white/50 tracking-wider transition-all duration-300 ${isHovered ? 'opacity-100 border-warm-white/40 text-warm-white/80' : 'opacity-0'}`}
              >
                {tag}
              </span>
            ))}
        ...
        {/* Bottom row */}
        <div>
          <p className="font-mono text-xs tracking-widest uppercase text-accent mb-2">{project.category}</p>
          <div className="flex items-end justify-between">
            ...
            >
              <span className="font-mono text-xs tracking-widest uppercase hidden sm:block">View Project</span>
